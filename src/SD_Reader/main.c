...
static esp_err_t sd_mount(const char* base_path, bool allow_format_if_needed) {
    esp_vfs_fat_sdmmc_mount_config_t mc = {
        .format_if_mount_failed = allow_format_if_needed, .max_files = 8, .allocation_unit_size = 16 * 1024};
...
static int list_csvs(void) {
    ...
    while ((e = readdir(d)) != NULL) {
        ...
        if (starts_with_icase(e->d_name, "dataLog") && ends_with_icase(e->d_name, ".csv")) {
            ...
            char numbuf[16] = {0};
            int k = 0;
            while (*p && k < (int) sizeof(numbuf) - 1 && isdigit((unsigned char) *p)) { numbuf[k++] = *p++; }
            if (k > 0) {
                int X = atoi(numbuf);
                if (X >= 0 && n < MAX_FILES) {
                    g_nums[n] = X;
...
static esp_err_t stream_csv_number(int X) {
    ...
    char buf[512];
    size_t r;
    while ((r = fread(buf, 1, sizeof(buf), fp)) > 0) fwrite(buf, 1, r, stdout);
    fclose(fp);
    printf("\n--- END %s ---\n", path);
    return ESP_OK;
}
...
void app_main(void) {
    ...
    while (1) {
        ...
        if ((line[0] == 'r' || line[0] == 'R') && (line[1] == 'm' || line[1] == 'M') && (line[2] == ' ')) {
        ...
        if (stream_csv_number(X) != ESP_OK) printf("Failed to stream dataLog%d.csv\n", X);
    }
}
