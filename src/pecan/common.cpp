...
int16_t setRTR(CANPacket *p) { // makes the given packet an RTR packet
    if (p->dataSize != 0) {
        return 1; // this packet has data written to it, it cant also be an rtr
                  // packet
    }
    p->rtr = 1;
    return 0;
...
int16_t
setExtended(CANPacket *p) { // makes the given packet an extended ID packet (so
                            // can send 29 bits of id, instead of just first 11)
    p->extendedID = 1;
    return 0;
}
...
copyValueToData(uint32_t *value, uint8_t *target, int8_t startBit,
                int8_t numBits) { // copies the first numBits of value into
                                  // target starting at startBit of target
    if (numBits <= 0 || startBit < 0 || startBit + numBits > 64)
        return 1; // function called in invalid circumstance

    // Treat target and source as 64-bit integers
    uint64_t *target64 = (uint64_t *)target;
    uint64_t source =
        (uint64_t)(*value) &
        ((1ULL << numBits) - 1); // Mask the relevant bits from value

    *target64 |= (source << startBit); // Shift the masked value to the correct
                                       // position and set it in target
    return 0;
}
...
copyDataToValue(uint32_t *target, uint8_t *data, int8_t startBit,
                int8_t numBits) { // relies on mc using little endian (since
                                  // interpretting array of bytes as int)
    if (numBits <= 0 || startBit < 0 || startBit + numBits > 64)
        return 1;
    ...
    uint64_t mask = ((1ULL << numBits) - 1)
                    << startBit; // Create a mask for the desired bit range
    uint64_t extractedBits =
        (*target64 & mask) >> startBit; // Extract and shift the relevant bits

    *target =
        (int32_t)extractedBits; // Update the value with the extracted bits
    return 0;
}
...
// of sending node, and last 4 bits being the function code
bool matchID(uint32_t id,
             uint32_t mask) { // check if the 7 bits of node ID must match mask
    return getNodeId(id) == getNodeId(mask);
}

bool matchFunction(
    uint32_t id,
    uint32_t mask) { // mask should contain 4 bit functoin code in bits 7-10.
    return getFunctionId(id) ==
           getFunctionId(mask); // only compares the functionCodes
}
