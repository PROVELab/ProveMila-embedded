...
// Simplified Can functionality Functions: feel free to never use these, and
// just use them as sample code for ESP-CAN library:
int16_t defaultPacketRecv(CANPacket* p) {
    if (xSemaphoreTake(*printfMutex, portMAX_DELAY)) {
        printf("Default handler: id %ld\n with data:", p->id);
        for (int i = 0; i < p->dataSize; i++) { printf("% d", (p->data)[i]); }
        printf("\n");
...
int16_t sendPacket(CANPacket* p) {
    if (p->dataSize > MAX_SIZE_PACKET_DATA) { return PACKET_TOO_BIG; }
    twai_message_t message = {
        // This is the struct used to create and send a CAN message. Generally
        // speaking, only the last 3 fields should ever change.
    ...
    esp_err_t err;
    while ((err = twai_transmit(&message, pdMS_TO_TICKS(30))) ==
           (ESP_FAIL)); // Continue trying to send indefinately in the event that we are
                        // waiting for another message to transfer (from another thread).
    // Otherwise give a timeout of 30ms for queue to open up. If the error is
    // something else, we are not in a state to transmit (which we should handle
    // when checking Bus State). The only other code is for invalid arguments,
    // in which case we will never be able to transmit anyway (although this
    // should fr never happen)

    return err;
}

// only for ESP, no arduino equivelant. to be scheduled as a task running every
// ~1s. Handles bus recovery. Sends status updates for changes in bus state asw.
    ...
    for (;;) {
        ...
        if (status_info.state == TWAI_STATE_BUS_OFF) {
            ...
            if (err != ESP_OK) { // restarts program in the event that we can't
                                 // start the Bus. This should never happen
                if (xSemaphoreTake(*printfMutex, portMAX_DELAY)) {
                    printf("Failed to start or restart driver with code: %d. "
                           "Rebooting\n",
                           err);
                    xSemaphoreGive(*printfMutex); // Release the mutex.
                } else {
