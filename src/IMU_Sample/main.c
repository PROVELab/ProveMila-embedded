...
icm20948_configure(icm20948_acce_fs_t acce_fs, icm20948_gyro_fs_t gyro_fs)
{
	...
	if (icm20948 == NULL) {
		printf("[ERROR] ICM20948 create returned NULL!\n");
		return ESP_FAIL;
	}
	printf("[INFO] ICM20948 creation successful!\n");

	ret = icm20948_reset(icm20948);
	if (ret != ESP_OK)
		return ret;
	printf("[INFO] ICM20948 reset successful!\n");
	...
	if (ret != ESP_OK)
		return ret;
	printf("[INFO] ICM20948 wake up successful!\n");

	ret = icm20948_set_bank(icm20948, 0);
	if (ret != ESP_OK)
		return ret;
	printf("[INFO] ICM20948 set bank successful!\n");

	uint8_t device_id;
	ret = icm20948_get_deviceid(icm20948, &device_id);
	if (ret != ESP_OK)
		return ret;
	printf("[INFO] WHO_AM_I register: 0x%02X\n", device_id);
...
{
	// printf("attempint icm config\n");
	esp_err_t ret = icm20948_configure(ACCE_FS_2G, GYRO_FS_1000DPS);
	if (ret != ESP_OK) {
		printf("[ERROR] ICM configuration failure\n");
		// while(1){
		// 	printf("broken!, %d\n", ret);
		// }
		vTaskDelete(NULL);
	}
	printf("[INFO] ICM20948 configuration successful!\n");
...
{  
	...
	printf("[INFO] I2C bus initialization: %s\n", esp_err_to_name(ret));
