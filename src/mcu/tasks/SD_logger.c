...
static void csv_header(FILE* fp){
    ...
    // motor_control_s
    fprintf(fp,
        "motor_control.current_reference,"
        "motor_control.discharge_limit_pct,"
        "motor_control.charge_limit_pct,");

    // motor_error_state
    fprintf(fp,
        "motor_error.motor_state,");

    // pedal_s
    fprintf(fp,
        "pedal.pedal_supply_voltage,"
...
static esp_err_t log_write_row(log_file_t *lf){
    ...
    if (fprintf(lf->fp,
            ...
            motor_power.measured_dc_voltage_v,
            motor_power.calculated_dc_current_a,
            motor_power.motor_current_limit_arms) < 0) return ESP_FAIL;
    ...
    if (fprintf(lf->fp,
            ...
            motor_speed.quadrature_current,
            motor_speed.direct_current,
            (int16_t)motor_speed.motor_speed) < 0) return ESP_FAIL;

    // ---- motor_safety_status_s
    if (fprintf(lf->fp,
            "%u,%u,%" PRId16 ",%" PRId16 ",%" PRId16 ",%u,",
            (unsigned)motor_safety.protection_code,
            (unsigned)motor_safety.safety_error_code,
            (int16_t)motor_safety.motor_temp,
            (int16_t)motor_safety.inverter_bridge_temp,
            (int16_t)motor_safety.bus_cap_temp,
            (unsigned)motor_safety.pwm_status) < 0) return ESP_FAIL;

    // ---- motor_control_s
    if (fprintf(lf->fp,
            "%" PRId32 ",%u,%u,",
            (int32_t)motor_control.current_reference,
            (unsigned)motor_control.discharge_limit_pct,
            (unsigned)motor_control.charge_limit_pct) < 0) return ESP_FAIL;

    // ---- motor_error_state
    if (fprintf(lf->fp,
            "%d,",
            (int)motor_error.motor_state) < 0) return ESP_FAIL;

    // ---- pedal_s
    if (fprintf(lf->fp,
            "%.6f,%.6f,%.6f,%.6f,%" PRId32 ",%d,",
            pedal_data.pedal_supply_voltage,
            pedal_data.pedal_position_pct,
            pedal_data.pedal_raw_1,
            pedal_data.pedal_raw_2,
            (int32_t)pedal_data.tx_value,
            (int)pedal_data.use_pedal) < 0) return ESP_FAIL;

    // ---- motor_protections_1_s
    if (fprintf(lf->fp,
            "%u,%u,%u,%u,%u,%u,%u,",
            (unsigned)motor_prot1.can_timeout_ms,
            (unsigned)motor_prot1.dc_regen_current_limit_neg_a,
            (unsigned)motor_prot1.dc_traction_current_limit_a,
            (unsigned)motor_prot1.stall_protection_type,
            (unsigned)motor_prot1.stall_protection_time_ms,
            (unsigned)motor_prot1.stall_protection_current_a,
            (unsigned)motor_prot1.overspeed_protection_speed_rpm) < 0) return ESP_FAIL;

    // ---- motor_protections_2_s (final chunk ends with \n)
    if (fprintf(lf->fp,
            "%u,%u,%u,%u,%u,%u,%u\n",
            (unsigned)motor_prot2.max_motor_temp_c,
            (unsigned)motor_prot2.motor_temp_high_gain_a_per_c,
            (unsigned)motor_prot2.max_inverter_temp_c,
            (unsigned)motor_prot2.inverter_temp_high_gain_a_per_c,
            (unsigned)motor_prot2.id_overcurrent_limit_a,
            (unsigned)motor_prot2.overvoltage_limit_v,
            (unsigned)motor_prot2.shutdown_voltage_limit_v) < 0) return ESP_FAIL;
