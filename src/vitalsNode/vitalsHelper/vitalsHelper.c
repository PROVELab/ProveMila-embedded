...
STATIC_ASSERT(warningFrameFlagIndex >= warningNodeFlagIndex,
              "warningFrameFlagIndex must be >= warningNodeFlagIndex");

// Helper to build safe masks without shifting by 32
static inline uint32_t mask_u32(unsigned bits) {
