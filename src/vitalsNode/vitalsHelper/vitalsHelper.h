...
inline void VitalsFlagSet(uint8_t nodeIndex, uint32_t bit) { 
    atomic_fetch_or(&(nodes[nodeIndex].flags), bit); 
}
inline void VitalsFlagClear(uint8_t nodeIndex, uint32_t bit) { 
    atomic_fetch_and(&(nodes[nodeIndex].flags), ~bit); 
}
inline uint8_t VitalsFlagsGet(uint8_t nodeIndex) { 
    return atomic_load(&(nodes[nodeIndex].flags)); 
}

inline void HBTimeSet(uint8_t nodeIndex, uint16_t time){
    atomic_store(&(nodes[nodeIndex].flags), time);
}
inline int16_t HBTimeGet(uint8_t nodeIndex){
    return atomic_load(&(nodes[nodeIndex].milliSeconds));
}
//
#endif