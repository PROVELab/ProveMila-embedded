...
static inline void updateSlowestIndex(uint32_t* slowestNodesArray, int16_t* worstTimesArray, int8_t nodeIndex, int16_t time){
...
void assertHBDefines(){
    ...
    STATIC_ASSERT(HBupdateTypeBits + HBTimerMSBits +
                slowestNodeCount * (nodeIDSizeBits + HBTimerMSBits) <= 64,
                "timing frame does not fit in 64 bits");
}

// two different frames for HB are sent. A timing frame and status frame(s) check diagram (should be in a memo at some point
// for details, of format, or look at the code
static void checkHB(void * pvParameters){
    ...
    for(;;){
        ...
        for(int i=0;i<numberOfNodes;i++){
            if(!HBStatusWriteMask){ //we have filled up this frame, lets send it!
                ...
                writeData(&message,(int8_t*)&data, sizeof(data));  //relies on esp32 being little endian (since interpretting uint64_t as array of bytes)
                sendPacket(&message);
            ...
            if(VitalsFlagsGet(i) & HBFlag){ //this is only place where HB flag gets cleared, so ok to check flag, then get time.
        ...
        for (int i = 0; i < slowestNodeCount && i < totalResponseCount; i++) {
            ...
            data |= ((uint64_t)slowNodeID << dataIndex); dataIndex += nodeIDSizeBits;
            data |= ((uint64_t)slowMs     << dataIndex); dataIndex += HBTimerMSBits;
        }

        //send timing frame:
        CANPacket timingMessage={0};
        timingMessage.id=combinedID(HBRespUpdate,vitalsID); 
        writeData(&timingMessage,(int8_t*)&data, sizeof(data));  //relies on esp32 being little endian (since interpretting uint64_t as array of bytes)
