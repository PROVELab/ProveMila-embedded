...
struct PTask {
    ...
    int16_t interval;        // Milliseconds between task runs, a negative interval indicates a task occruing one time, -interval ms after its declared
    int16_t location;        // Location of Task in task array, assigned upon task scheduling
...
private:
    ...
    //PTask tasks[MAX_TASK_COUNT];    //delete wen decide not to use originally scheduling
    //int16_t dCtr=0;
